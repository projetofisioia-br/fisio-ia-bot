...
from pymongo import MongoClient # NOVO BLOCO
...
MODELO = "gemini-1.5-flash" # Sugestão: 1.5-flash é a estável atual
MONGO_URI = os.environ.get("MONGO_URI") # NOVO BLOCO
TOKEN_PAYMENT = os.environ.get("TOKEN_PAYMENT") # NOVO BLOCO

# CONEXÃO BANCO DE DADOS (NOVO BLOCO)
...
def menu_principal():
    ...
    btn_historico = types.InlineKeyboardButton("📂 Histórico de Pacientes", callback_data="ver_historico") # NOVO BLOCO
    btn_duvida = types.InlineKeyboardButton("📚 Dúvida Técnica", callback_data="duvida_tecnica")
    btn_planos = types.InlineKeyboardButton("💎 Planos de Acesso Pro", callback_data="planos") # NOVO BLOCO
    markup.add(btn_paciente, btn_historico, btn_duvida, btn_planos) # ATUALIZADO
    return markup
...
def processar_ia_paciente(message, nome):
    ...
    chamar_gemini(message, prompt, nome) # ATUALIZADO: Passando o nome

def processar_ia_direta(message):
    prompt = f"{PROMPT_SISTEMA}\n\nForneça uma explanação técnica PhD sobre: {message.text}"
    chamar_gemini(message, prompt)

def chamar_gemini(message, prompt, nome_paciente=None): # ATUALIZADO: Aceita nome
    ...
    try:
        # Payload com desativação de filtros para evitar bloqueios de segurança
        payload = {
